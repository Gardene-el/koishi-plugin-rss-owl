...
import { SecurityError, getSecurityOptions, validateUrlOrThrow } from '../utils/security'
...
async function processCustomTemplate(
  ...
  const renderedDescription = await renderTemplatedDescription(deps, item, arg, description)
...
async function processContentTemplate(
  ...
  item.description = contentText.replace(/\$img\{\{(.*?)\}\}/g, (_match, src: string) => {
...
async function processLinkTemplate(
  ...
  if (!src) {
    debug(deps.config, 'link 模板未找到可用链接，回退原始内容', 'link src', 'info')
  ...
  try {
    ...
    if (error instanceof SecurityError) {
      debug(deps.config, `链接 URL 安全验证失败: ${error.message}`, 'security', 'error')
